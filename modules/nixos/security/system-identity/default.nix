# https://forge.lel.lol/patrick/nix-config/src/commit/d8540f5279877775670cb9f7d2d9e6136953811e/modules/ensure-pcr.nix
{
  lib,
  config,
  pkgs,
...
let
  inherit (lib)
    ...
    ;
  inherit
    (import "${inputs.nixpkgs}/nixos/lib/utils.nix" (lib.getAttrs [ "lib" "pkgs" "config" ] args))
    escapeSystemdPath
    ;
in
{
  options = {
    systemIdentity = {
      ...
      soft = mkOption {
        ...
        default = false;
        description = ''
          If set to true, the boot will not abort if the check fails.
          Instead, it will just log a warning.
        '';
      };
      pcr15 = mkOption {
      ...
      check-pcrs = mkIf (config.systemIdentity.pcr15 != null) {
        script = ''
          ...
          if [[ $(systemd-analyze pcrs 15 --json=short | jq -r ".[0].sha256") != "${config.systemIdentity.pcr15}" ]] ; then
            ...
            ${lib.optionalString (!config.systemIdentity.soft) "exit 1"}
          else
            echo "PCR 15 check success"
          fi
        '';
        serviceConfig = {
          Type = "oneshot";
        ...
        [
          (nameValuePair "cryptsetup-${attrs.name}" {
            unitConfig = {
              ...
              BindsTo = "${escapeSystemdPath attrs.value.device}.device";
            };
            serviceConfig = {
              Type = "oneshot";
              RemainAfterExit = true;
            ...
            after = [
              ...
              "${escapeSystemdPath attrs.value.device}.device"
            ]
